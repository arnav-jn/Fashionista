...
read_objfiles = ['csv-data/tshirts/colour_top_bottom.csv',
                 'csv-data/tshirts/neck_top_bottom.csv',
                 'csv-data/tshirts/print_top_bottom.csv']
dataset_csv_files = 'csv-data/tshirts/tshirts_csv_final.csv'
...
for ite in range(0, len(write_objfiles)):
    with open(read_objfiles[ite], 'r') as read_obj:
        dataset_csv = pd.read_csv(dataset_csv_files, names=colnames, delimiter=',', error_bad_lines=False, header=None,
                                  usecols=reqdcolnames, na_values=" NaN")
